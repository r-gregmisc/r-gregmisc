...
venn.observed2expected <- function(counts,new.total=NA)
{
  ...
  expected <- apply(counts[,-1],1,function(X) {
    #cat("X:\n"); print(X)
  ...
  return(cbind(num=expected,counts[,-1])) 
}

#counts.expected  <- observed2expected(counts)
#counts.expected2 <- observed2expected(counts.expected) # should project on itself, and it does
